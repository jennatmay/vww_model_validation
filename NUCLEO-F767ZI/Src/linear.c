...
#define AI_NET_OBJ_INSTANCE g_linear
 
#undef AI_LINEAR_MODEL_SIGNATURE
#define AI_LINEAR_MODEL_SIGNATURE     "0xbd95d6f1c3b4fe85e704b52ee23cc81c"
...
#define AI_TOOLS_DATE_TIME   "2024-11-11T00:20:07-0600"

#undef AI_TOOLS_COMPILE_TIME
#define AI_TOOLS_COMPILE_TIME    __DATE__ " " __TIME__

#undef AI_LINEAR_N_BATCHES
#define AI_LINEAR_N_BATCHES         (1)

static ai_ptr g_linear_activations_map[1] = AI_C_ARRAY_INIT;
static ai_ptr g_linear_weights_map[1] = AI_C_ARRAY_INIT;
...
/* Array#1 */
AI_ARRAY_OBJ_DECLARE(
  gemm_0_output_array, AI_ARRAY_FORMAT_FLOAT|AI_FMT_FLAG_IS_IO,
  NULL, NULL, 1, AI_STATIC)
...
AI_NETWORK_OBJ_DECLARE(
  ...
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, 8, 1, 1),
    8, NULL, NULL),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_LINEAR_IN_NUM, &serving_default_input_layer0_output),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_LINEAR_OUT_NUM, &gemm_0_output),
  &gemm_0_layer, 0x8dd69508, NULL)
...
/******************************************************************************/
AI_DECLARE_STATIC
ai_bool linear_configure_activations(
...
/******************************************************************************/
AI_DECLARE_STATIC
ai_bool linear_configure_weights(
  ai_network* net_ctx, const ai_network_params* params)
{
  AI_ASSERT(net_ctx)
  ...
  ai_handle network, ai_network_report* report)
{
  ...
  {
    ai_network_report r = {
      .model_name        = AI_LINEAR_MODEL_NAME,
  ...
  ai_handle* network, const ai_handle activations[], const ai_handle weights[])
{
  ...
  err = ai_linear_create(network, AI_LINEAR_DATA_CONFIG);
  if (err.type != AI_ERROR_NONE) {
    return err;
  }
  
  if (ai_linear_data_params_get(&params) != true) {
    err = ai_linear_get_error(*network);
    return err;
  }
#if defined(AI_LINEAR_DATA_ACTIVATIONS_COUNT)
  /* set the addresses of the activations buffers */
  for (ai_u16 idx=0; activations && idx<params.map_activations.size; idx++) {
    AI_BUFFER_ARRAY_ITEM_SET_ADDRESS(&params.map_activations, idx, activations[idx]);
  }
#endif
#if defined(AI_LINEAR_DATA_WEIGHTS_COUNT)
  /* set the addresses of the weight buffers */
  for (ai_u16 idx=0; weights && idx<params.map_weights.size; idx++) {
    AI_BUFFER_ARRAY_ITEM_SET_ADDRESS(&params.map_weights, idx, weights[idx]);
  }
#endif
  if (ai_linear_init(*network, &params) != true) {
    err = ai_linear_get_error(*network);
  }
  return err;
}
...
AI_API_ENTRY
ai_buffer* ai_linear_inputs_get(ai_handle network, ai_u16 *n_buffer)
{
...
AI_API_ENTRY
ai_buffer* ai_linear_outputs_get(ai_handle network, ai_u16 *n_buffer)
{
  if (network == AI_HANDLE_NULL) {
    network = (ai_handle)&AI_NET_OBJ_INSTANCE;
    AI_NETWORK_OBJ(network)->magic = AI_MAGIC_CONTEXT_TOKEN;
