...
#define AI_NET_OBJ_INSTANCE g_vww_int8_keras
 
#undef AI_VWW_INT8_KERAS_MODEL_SIGNATURE
#define AI_VWW_INT8_KERAS_MODEL_SIGNATURE     "0x8033b904830c62911d871fdee80a8a00"
...
#define AI_TOOLS_DATE_TIME   "2024-11-11T15:35:36-0600"

#undef AI_TOOLS_COMPILE_TIME
#define AI_TOOLS_COMPILE_TIME    __DATE__ " " __TIME__

#undef AI_VWW_INT8_KERAS_N_BATCHES
#define AI_VWW_INT8_KERAS_N_BATCHES         (1)

static ai_ptr g_vww_int8_keras_activations_map[1] = AI_C_ARRAY_INIT;
static ai_ptr g_vww_int8_keras_weights_map[1] = AI_C_ARRAY_INIT;
...
AI_NETWORK_OBJ_DECLARE(
  ...
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    ...
    153696, NULL, NULL),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_VWW_INT8_KERAS_IN_NUM, &input_1_output),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_VWW_INT8_KERAS_OUT_NUM, &dense_output),
  &conv2d_layer, 0x496a7465, NULL)
...
/******************************************************************************/
AI_DECLARE_STATIC
ai_bool vww_int8_keras_configure_activations(
...
/******************************************************************************/
AI_DECLARE_STATIC
ai_bool vww_int8_keras_configure_weights(
  ai_network* net_ctx, const ai_network_params* params)
{
  AI_ASSERT(net_ctx)
  ...
  ai_handle network, ai_network_report* report)
{
  ...
  {
    ai_network_report r = {
      .model_name        = AI_VWW_INT8_KERAS_MODEL_NAME,
  ...
  ai_handle* network, const ai_handle activations[], const ai_handle weights[])
{
  ...
  err = ai_vww_int8_keras_create(network, AI_VWW_INT8_KERAS_DATA_CONFIG);
  if (err.type != AI_ERROR_NONE) {
    return err;
  }
  
  if (ai_vww_int8_keras_data_params_get(&params) != true) {
    err = ai_vww_int8_keras_get_error(*network);
    return err;
  }
#if defined(AI_VWW_INT8_KERAS_DATA_ACTIVATIONS_COUNT)
  /* set the addresses of the activations buffers */
  for (ai_u16 idx=0; activations && idx<params.map_activations.size; idx++) {
    AI_BUFFER_ARRAY_ITEM_SET_ADDRESS(&params.map_activations, idx, activations[idx]);
  }
#endif
#if defined(AI_VWW_INT8_KERAS_DATA_WEIGHTS_COUNT)
  /* set the addresses of the weight buffers */
  for (ai_u16 idx=0; weights && idx<params.map_weights.size; idx++) {
    AI_BUFFER_ARRAY_ITEM_SET_ADDRESS(&params.map_weights, idx, weights[idx]);
  }
#endif
  if (ai_vww_int8_keras_init(*network, &params) != true) {
    err = ai_vww_int8_keras_get_error(*network);
  }
  return err;
}
...
AI_API_ENTRY
ai_buffer* ai_vww_int8_keras_inputs_get(ai_handle network, ai_u16 *n_buffer)
{
...
AI_API_ENTRY
ai_buffer* ai_vww_int8_keras_outputs_get(ai_handle network, ai_u16 *n_buffer)
{
  if (network == AI_HANDLE_NULL) {
    network = (ai_handle)&AI_NET_OBJ_INSTANCE;
    AI_NETWORK_OBJ(network)->magic = AI_MAGIC_CONTEXT_TOKEN;
